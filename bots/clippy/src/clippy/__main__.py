"""Main entry point for the Clippy bot."""
...
from .bot import ClippyBot


async def main() -> None:
    """Main function to run the Clippy bot."""
    ...
    try:
        ...
        if bot_task in done and not bot_task.cancelled():
            try:
                ...
                logger.error("Clippy bot failed", error=str(e))
                sys.exit(1)
                
    except Exception as e:
        logger.error("Failed to start Clippy bot", error=str(e))
        await bot.close()
        sys.exit(1)
